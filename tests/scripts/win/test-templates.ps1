...
Write-Output "`ndso template list -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" --namespace-scope --uninherited | dso template delete -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" --namespace-scope -i -`n"
Invoke-Call -ScriptBlock {dso template list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" --namespace-scope --uninherited | dso template delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" --namespace-scope -i -} -ErrorAction Stop > $null

Write-Output "`ndso template list -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s ${stage} --namespace-scope --uninherited | dso template delete -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s ${stage} --namespace-scope -i -`n"
Invoke-Call -ScriptBlock {dso template list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s ${stage} --namespace-scope --uninherited | dso template delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s ${stage} --namespace-scope -i -} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" --uninherited | dso template delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -i -} -ErrorAction Stop > $null

Write-Output "`ndso template list -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s ${stage} --uninherited | dso template delete -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s ${stage} -i -`n"
Invoke-Call -ScriptBlock {dso template list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s ${stage} --uninherited | dso template delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s ${stage} -i -} -ErrorAction Stop > $null

Write-Output "`ndso template list -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s `"${stage}/2`" --uninherited | dso template delete -b5 -w `"${working_dir}`" --namespace ${namespace} --namespace ${namespace} --application ${application} --config `"template.provider.id=${provider}`" -s `"${stage}/2`" -i -`n"
Invoke-Call -ScriptBlock {dso template list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s "${stage}/2" --uninherited | dso template delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, template.provider.id=${provider}" -s "${stage}/2" -i -} -ErrorAction Stop > $null
...
Write-Output "`ndso template add -b5 -w `"${working_dir}`" namespace.template -r 'tests\output\template\*' -c tests\sample-templates\namespace-template --namespace-scope`n"
Invoke-Call -ScriptBlock {dso template add -b5 -w "${working_dir}" namespace.template -r 'tests\output\template\*' -c tests\sample-templates\namespace-template --namespace-scope} -ErrorAction Stop > $null

Write-Output "`ndso template add -b5 -w `"${working_dir}`" -s ${stage} namespace.stage_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-stage-template --namespace-scope`n"
Invoke-Call -ScriptBlock {dso template add -b5 -w "${working_dir}" -s ${stage} namespace.stage_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-stage-template --namespace-scope} -ErrorAction Stop > $null
...
Write-Output "`ndso template add -b5 -w `"${working_dir}`" overriden_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-template-overriden --namespace-scope`n"
Invoke-Call -ScriptBlock {dso template add -b5 -w "${working_dir}" overriden_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-template-overriden --namespace-scope} -ErrorAction Stop > $null

Write-Output "`ndso template add -b5 -w `"`n${working_dir}`" -s ${stage} overriden_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-stage-template-overriden --namespace-scope`n"
Invoke-Call -ScriptBlock {dso template add -b5 -w "${working_dir}" -s ${stage} overriden_template -r 'tests\output\template\*' -c tests\sample-templates\namespace-stage-template-overriden --namespace-scope} -ErrorAction Stop > $null
...
Write-Output "`ndso template get -b5 -w `"${working_dir}`" overriden_template --scope Namespace -f raw`n"
Invoke-Call -ScriptBlock {dso template get -b5 -w "${working_dir}" overriden_template --scope Namespace -f raw} -ErrorAction Stop > $null
...
Write-Output "`ndso template edit -b5 -w `"${working_dir}`" overriden_template -s ${stage} --namespace-scope`n"
Invoke-Call -ScriptBlock {dso template edit -b5 -w "${working_dir}" overriden_template -s ${stage} --namespace-scope} -ErrorAction Stop
