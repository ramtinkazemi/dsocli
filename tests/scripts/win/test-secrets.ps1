...
Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --global-scope --uninherited -f json | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --global-scope -i - -f json`n"
...
Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --project-scope --uninherited | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --project-scope -i -`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" --project-scope --uninherited | dso secret delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" --project-scope -i -} -ErrorAction Stop > $null

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s ${stage} --project-scope --uninherited -f shell | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s ${stage} --project-scope -i - -f shell`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s ${stage} --project-scope --uninherited -f shell | dso secret delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s ${stage} --project-scope -i - -f shell} -ErrorAction Stop > $null

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --uninherited | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -i -`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" --uninherited | dso secret delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -i -} -ErrorAction Stop > $null

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s ${stage} --uninherited | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s ${stage} -i -`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s ${stage} --uninherited | dso secret delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s ${stage} -i -} -ErrorAction Stop > $null

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s `"${stage}/2`" --uninherited | dso secret delete -b5 -w `"${working_dir}`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" -s `"${stage}/2`" -i -`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s "${stage}/2" --uninherited | dso secret delete -b5 -w "${working_dir}" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" -s "${stage}/2" -i -} -ErrorAction Stop > $null

##################################
### add context-specific secrets

Write-Output "`nWrite-Output `"global.secret=global_secret`" | dso secret add -b5 -w `"${working_dir}`" --global-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "global.secret=global_secret" | dso secret add -b5 -w "${working_dir}" --global-scope -f shell -i -} -ErrorAction Stop > $null

Write-Output "`nWrite-Output `"global.stage_secret=global_stage_secret`" | dso secret add -b5 -w `"${working_dir}`" -s ${stage} --global-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "global.stage_secret=global_stage_secret" | dso secret add -b5 -w "${working_dir}" -s ${stage} --global-scope -f shell -i -} -ErrorAction Stop > $null

Write-Output "`nWrite-Output `"project.secret=project_secret`" | dso secret add -b5 -w `"${working_dir}`" --project-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "project.secret=project_secret" | dso secret add -b5 -w "${working_dir}" --project-scope -f shell -i -} -ErrorAction Stop > $null
...
Write-Output "`nWrite-Output `"overriden_secret=global_overriden_secret`" | dso secret add -b5 -w `"${working_dir}`" --global-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "overriden_secret=global_overriden_secret" | dso secret add -b5 -w "${working_dir}" --global-scope -f shell -i -} -ErrorAction Stop > $null

Write-Output "`nWrite-Output `"overriden_secret=global_stage_overriden_secret`" | dso secret add -b5 -w `"${working_dir}`" -s ${stage} --global-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "overriden_secret=global_stage_overriden_secret" | dso secret add -b5 -w "${working_dir}" -s ${stage} --global-scope -f shell -i -} -ErrorAction Stop > $null

Write-Output "`nWrite-Output `"overriden_secret=project_overriden_secret`" | dso secret add -b5 -w `"${working_dir}`" --project-scope -f shell -i -`n"
Invoke-Call -ScriptBlock {Write-Output "overriden_secret=project_overriden_secret" | dso secret add -b5 -w "${working_dir}" --project-scope -f shell -i -} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso secret add overriden_secret app_stage2_overriden_secret -b5 -w "${working_dir}" -s "${stage}/2"} -ErrorAction Stop > $null

##################################
### get some secrets

Write-Output "`ndso secret get -b5 -w `"${working_dir}`" overriden_secret --scope Global -f raw`n"
Invoke-Call -ScriptBlock {dso secret get -b5 -w "${working_dir}" overriden_secret --scope Global -f raw} -ErrorAction Stop > $null

Write-Output "`ndso secret get -b5 -w `"${working_dir}`" overriden_secret --scope Namespace -f raw`n"
Invoke-Call -ScriptBlock {dso secret get -b5 -w "${working_dir}" overriden_secret --scope Namespace -f raw} -ErrorAction Stop > $null
...
Write-Output "`ndso secret get -b5 -w `"${working_dir}`" overriden_secret -s `"${stage}/2`" -f raw`n"
Invoke-Call -ScriptBlock {dso secret get -b5 -w "${working_dir}" overriden_secret -s "${stage}/2" -f raw} -ErrorAction Stop > $null
...
Write-Output "`ndso secret get -b5 -w `"${working_dir}`" app.stage2_secret -s `"${stage}/2`" -f raw`n"
Invoke-Call -ScriptBlock {dso secret get -b5 -w "${working_dir}" app.stage2_secret -s "${stage}/2" -f raw} -ErrorAction Stop > $null
...
Write-Output "`ndso secret edit -b5 -w `"${working_dir}`" app.stage2_secret -s `"${stage}/2`"`n"
Invoke-Call -ScriptBlock {dso secret edit -b5 -w "${working_dir}" app.stage2_secret -s "${stage}/2"} -ErrorAction Stop
...
Write-Output "`ndso secret history -b5 -w `"${working_dir}`" app.stage2_secret -s `"${stage}/2`" --query-all -f json`n"
Invoke-Call -ScriptBlock {dso secret history -b5 -w "${working_dir}" app.stage2_secret -s "${stage}/2" --query-all -f json} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" -s ${stage} --uninherited --query-all -f json} -ErrorAction Stop > "tests/output/secret/app.stage_uninherited-${filename}.json"

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" -s ${stage} --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --query-all -f yaml`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" -s ${stage} --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" --query-all -f yaml} -ErrorAction Stop > "tests/output/secret/app.stage_all-${filename}.yaml"

Write-Output "`ndso secret list -b5 -w `"${working_dir}`" -s `"${stage}/2`" --config `"namespace=${namespace}, application=${application}, secret.provider.id=${provider}`" --query-all -f yaml`n"
Invoke-Call -ScriptBlock {dso secret list -b5 -w "${working_dir}" -s "${stage}/2" --config "namespace=${namespace}, application=${application}, secret.provider.id=${provider}" --query-all -f yaml} -ErrorAction Stop > "tests/output/secret/app.stage2_all-${filename}.yaml"
