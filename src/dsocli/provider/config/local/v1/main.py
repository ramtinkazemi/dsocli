...
class LocalConfigProvider(ConfigProvider):
    ...
    def get(self, service, key, revision=None, uninherited=False, editable=False):
        ...
        Logger.debug(f"Getting configuration setting '{key}': namespace={AppConfigs.get_namespace(ContextSource.Target)}, application={AppConfigs.get_application(ContextSource.Target)}, stage={AppConfigs.get_stage(ContextSource.Target)}, scope={AppConfigs.scope}")
        ...
        if not found:
            if uninherited:
                raise DSOException(f"Setting '{key}' not found in the given context: namespace={AppConfigs.get_namespace(ContextSource.Target)}, application={AppConfigs.get_application(ContextSource.Target)}, stage={AppConfigs.get_stage(ContextSource.Target)}, scope={AppConfigs.scope}")
            else:
                raise DSOException(f"Setting '{key}' not found nor inherited in the given context: namespace={AppConfigs.get_namespace(ContextSource.Target)}, application={AppConfigs.get_application(ContextSource.Target)}, stage={AppConfigs.get_stage(ContextSource.Target)}, scope={AppConfigs.scope}")
        # if len(found) > 1:
    ...
    def history(self, service, key):
        ...
        Logger.debug(f"Getting local config setting '{key}': namespace={AppConfigs.get_namespace(ContextSource.Target)}, application={AppConfigs.get_application(ContextSource.Target)}, stage={AppConfigs.get_stage(ContextSource.Target)}, scope={AppConfigs.scope}")
    ...
    def unset(self, service, key):
        self.service = service
        Logger.debug(f"Locating config setting '{key}': namespace={AppConfigs.get_namespace(ContextSource.Target)}, application={AppConfigs.get_application(ContextSource.Target)}, stage={AppConfigs.get_stage(ContextSource.Target)}, scope={AppConfigs.scope}")
