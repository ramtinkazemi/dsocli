...
class AwsSsmSecretProvider(SecretProvider):
    ...
    def list(self, uninherited=False, decrypt=False, filter=None):
        Logger.debug(f"Listing SSM secrets: namesape={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
    ...
    def add(self, key, value):
        Logger.debug(f"Checking SSM secret overwrites '{key}': namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
        assert_ssm_parameter_no_namespace_overwrites(key=key, path_prefix=self.get_path_prefix())
        Logger.debug(f"Locating SSM secret '{key}': namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
        ...
        if found and not found['Type'] == 'SecureString':
            raise DSOException(f"Failed to add secret '{key}' becasue becasue the key is not available in the given context: namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.short_stage}")
    ...
    def get(self, key, revision=None, uninherited=False, decrypt=False):
        ...
        if not found:
            ...
            if not found['Type'] == 'SecureString':
                raise DSOException(f"Secret '{key}' not found in the given context: namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.short_stage}")
        ...
        if revision is None:
            ...
            if not secrets:
                raise DSOException(f"Revision '{revision}' not found for secret '{key}' in the given context: namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.short_stage}")
            result = {
                    'RevisionId': str(secrets[0]['Version']),
                    'Date': secrets[0]['LastModifiedDate'].strftime('%Y/%m/%d-%H:%M:%S'),
                    'Key': key, 
    ...
    def history(self, key, decrypt=False):
        Logger.debug(f"Locating SSM secret '{key}': application={AppConfig.application}, stage={AppConfig.stage}")
