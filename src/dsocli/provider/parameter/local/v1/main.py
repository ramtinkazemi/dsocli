...
class LocalParameterProvider(ParameterProvider):
    ...
    def add(self, key, value):
        Logger.debug(f"Adding local parameter '{key}': namespace={Config.get_namespace(ContextMode.Target)}, application={Config.get_application(ContextMode.Target)}, stage={Config.get_stage(ContextMode.Target)}, scope={Config.scope}")
    ...
    def list(self, uninherited=False, filter=None):
        Logger.debug(f"Listing local parameters: namespace={Config.get_namespace(ContextMode.Target)}, application={Config.get_application(ContextMode.Target)}, stage={Config.get_stage(ContextMode.Target)}, scope={Config.scope}")
    ...
    def get(self, key, revision=None, uninherited=False, rendered=True):
        ...
        Logger.debug(f"Getting local parameter '{key}': namespace={Config.get_namespace(ContextMode.Target)}, application={Config.get_application(ContextMode.Target)}, stage={Config.get_stage(ContextMode.Target)}, scope={Config.scope}")
    ...
    def history(self, key):
        Logger.warn(f"Parameter provider 'local/v1' does not support history.")
