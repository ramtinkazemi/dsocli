...
__default_spec = {
    'path': os.path.join(AppConfig.config_dir, 'parameter/'),
    # 'namespace': 'default',
    'store': 'local.json',
}
...
class LocalParameterProvider(ParameterProvider):
    ...
    def add(self, key, value):
        Logger.debug(f"Adding local parameter '{key}': namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
    ...
    def list(self, uninherited=False, filter=None):
        Logger.debug(f"Listing local parameters: namespace={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
    ...
    def get(self, key, revision=None, uninherited=False, editable=False):
        ...
        Logger.debug(f"Getting local parameter '{key}': namesape={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
    ...
    def delete(self, key):
        Logger.debug(f"Locating parameter '{key}': namesape={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.stage}")
        ...
        if not found:
            raise DSOException(f"Parameter '{key}' not found in the given context: namesape={AppConfig.namespace}, application={AppConfig.application}, stage={AppConfig.short_stage}")
