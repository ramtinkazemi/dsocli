...
Invoke-Call -ScriptBlock {dso template list -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" --uninherited -f json | dso template delete -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" -i -} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template list -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" -s $stage --uninherited -f json | dso template delete -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" -s $stage -i -} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template list -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" -s "$stage/2" --uninherited -f json | dso template delete -v6 --config "namespace=$namespace, application=$application, template.provider.id=$provider" -s "$stage/2" -i -} -ErrorAction Stop > $null
...
Write-Output "`ndso config add -v6 template.provider.id $provider`n"
Invoke-Call -ScriptBlock {dso config add -v6 template.provider.id "$provider"} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\global-template global_template -r '.dso\output\template\*' --global-scope -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\global-stage-template global_stage_template -r '.dso\output\template\*' -s $stage --global-scope -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\namespace-template namespace_template -r '.dso\output\template\*' --namespace-scope -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\namespace-stage-template namespace_stage_template -r '.dso\output\template\*' -s $stage --namespace-scope -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\namespace-stage-template namespace_stage_template -r '.dso\output\template\*' -s $stage --namespace-scope -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\app-template app_template -r '.dso\output\template\*' -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\app-template app_template -r '.dso\output\template\*' -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\app-stage-template app_stage_template -r '.dso\output\template\*' -s $stage -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\app-stage-template  app_stage_template -r '.dso\output\template\*' -s $stage -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\app-stage2-template app_stage2_template -r '.dso\output\template\*' -s "$stage/2" -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\global-template-overriden overriden_template -r '.dso\output\template\*' --global-scope -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\global-stage-template-overriden overriden_template -r '.dso\output\template\*' -s $stage --global-scope -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\namespace-template-overriden overriden_template -r '.dso\output\template\*' --namespace-scope -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\namespace-template-overriden overriden_template -r '.dso\output\template\*' --namespace-scope -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\namespace-stage-template-overriden overriden_template -r '.dso\output\template\*' -s $stage --namespace-scope -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\namespace-stage-template-overriden overriden_template -r '.dso\output\template\*' -s $stage --namespace-scope -v6} -ErrorAction Stop > $null

Write-Output "`ndso template add sample-templates\app-template-overriden overriden_template -r '.dso\output\template\*' -v6`n"
Invoke-Call -ScriptBlock {dso template add sample-templates\app-template-overriden overriden_template -r '.dso\output\template\*' -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\app-stage-template-overriden overriden_template -r '.dso\output\template\*' -s $stage -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template add sample-templates\app-stage2-template-overriden overriden_template -r '.dso\output\template\*' -s "$stage/2" -v6} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template get -v6 overriden_template --scope Global -f text} -ErrorAction Stop > $null

Write-Output "`ndso template get -v6 overriden_template --scope Namespace -f text`n"
Invoke-Call -ScriptBlock {dso template get -v6 overriden_template --scope Namespace -f text} -ErrorAction Stop > $null

Write-Output "`ndso template get -v6 overriden_template -s $stage -f text`n"
Invoke-Call -ScriptBlock {dso template get -v6 overriden_template -s $stage -f text} -ErrorAction Stop > $null

Write-Output "`ndso template get -v6 app_template -f text`n"
Invoke-Call -ScriptBlock {dso template get -v6 app_template -f text} -ErrorAction Stop > $null

Write-Output "`ndso template get -v6 app_stage_template -s $stage -f text`n"
Invoke-Call -ScriptBlock {dso template get -v6 app_stage_template -s $stage -f text} -ErrorAction Stop > $null

Write-Output "`ndso template get -v6 app_stage2_template -s `"$stage/2`" -f text`n"
Invoke-Call -ScriptBlock {dso template get -v6 app_stage2_template -s "$stage/2" -f text} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template history -v6 overriden_template -g -f json} -ErrorAction Stop > $null

Write-Output "`ndso template history -v6 overriden_template -n -f json`n"
Invoke-Call -ScriptBlock {dso template history -v6 overriden_template -n -f json} -ErrorAction Stop > $null

Write-Output "`ndso template history -v6 overriden_template -s $stage -f yaml`n"
Invoke-Call -ScriptBlock {dso template history -v6 overriden_template -s $stage -f json} -ErrorAction Stop > $null

Write-Output "`ndso template history -v6 app_template -s $stage --query-all -f json`n"
Invoke-Call -ScriptBlock {dso template history -v6 app_template -s $stage --query-all -f json} -ErrorAction Stop > $null

Write-Output "`ndso template history -v6 app_stage_template -s $stage --query-all -f yaml`n"
Invoke-Call -ScriptBlock {dso template history -v6 app_stage_template -s $stage --query-all -f json} -ErrorAction Stop > $null

Write-Output "`ndso template history -v6 app_stage2_template -s `"$stage/2`" --query-all -f yaml`n"
Invoke-Call -ScriptBlock {dso template history -v6 app_stage2_template -s "$stage/2" --query-all -f json} -ErrorAction Stop > $null
...
Invoke-Call -ScriptBlock {dso template render overriden_template -v6} -ErrorAction Stop > $null
